...
let photos = `CREATE TABLE if not exists photos(
  id INT NOT NULL AUTO_INCREMENT,
  name VARCHAR(100) NOT NULL COMMENT '用户名',
  title VARCHAR(100) NOT NULL DEFAULT null  COMMENT '图片标题',
  desccon VARCHAR(300) NOT NULL DEFAULT null COMMENT '图片描述',
  path VARCHAR(100) NOT NULL COMMENT '图片路径',
  uploadtime VARCHAR(100) NOT NULL COMMENT '上传时间',
  PRIMARY KEY (id)
)`;
